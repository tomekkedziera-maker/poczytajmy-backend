...
app.get('/health', (_req, res) => {
  res.json({ ok: true, service: 'poczytajmy-backend', version: '1.14-object-priority' });
});
...
/* === NOWE: 1os twarde dopasowanie i ekstrakcja dopełnienia === */
function findTransitive1stFormHard(text="") {
  const s = String(text).trim().toLowerCase();
  let m = s.match(/^(ja\s+)?(czytam|jem|piję|oglądam|słucham|gram)\b/i);
...
function extractObjectAfter1st(text="", verbForm="") {
  ...
  if (verbForm === "gram") {
    return seg.replace(/\s*[,;:].*$/u, "").trim(); // np. "w piłkę"
  }

  const cut = seg.split(/\s+(?:(?!o\b)(w|na|przy|pod|obok|koło|u|do|z|ze|po))\s+/i)[0];
  return (cut || seg).replace(/\s*[,;:].*$/u, "").trim();
}

/* === NOWE: 3os prosta heurystyka (Imię + czasownik + dopełnienie) === */
function thirdPersonDirectQA(text="") {
  const s = String(text).trim();
  const m = s.match(/\b([A-ZŁŚŻŹĆŃ][a-ząćęłńóśźż]+)\s+(czyta|ogląda|słucha|je|pije|gra)\b\s+([^.!?]+)/u);
  if (!m) return null;
  ...
  if (verb === "słucha") {
    const obj = rest.split(/\s+(?:(?!o\b)(w|na|przy|pod|obok|koło|u|do|z|ze|po))\s+/i)[0].replace(/\s*[,;:].*$/u,"").trim();
    if (obj) return { question: `Czego słucha ${name}?`, answer: obj };
    return null;
  }
  const obj = rest.split(/\s+(?:(?!o\b)(w|na|przy|pod|obok|koło|u|do|z|ze|po))\s+/i)[0].replace(/\s*[,;:].*$/u,"").trim();
  if (!obj) return null;
  const qVerb = (verb === "czyta") ? "czyta" : (verb === "ogląda") ? "ogląda" : (verb === "je") ? "je" : (verb === "pije") ? "pije" : "robi";
...
app.post('/agent/comprehend', async (req, res) => {
  try {
    ...
    // === 1. OSOBA — PRIORYTET: dopełnienie (Co/Czego/W co) ===
    function buildFirstPersonQA(txt) {
      const hard = findTransitive1stFormHard(txt);
      if (hard) {
        const obj = extractObjectAfter1st(txt, hard);
        if (obj) {
          if (hard === "słucham") return { question: `Czego ${hard}?`, answer: obj };
          if (hard === "gram")    return { question: `W co ${hard}?`,  answer: obj };
          return { question: `Co ${hard}?`, answer: obj }; // czytam/jem/piję/oglądam
        }
      }

      // Cel → Czas → Miejsce → fallback
      const purpose = (String(txt).match(/\b(żeby|aby)\s+[^.?!,]+/i)?.[0] || '').trim();
      const when =
        (String(txt).match(/\b(jutro|dzisiaj|dziś|wczoraj|rano|wieczorem|po południu|popołudniu)\b/i)?.[0] ||
         String(txt).match(/\bw\s+(poniedziałek|wtorek|środę|czwartek|piątek|sobotę|niedzielę)\b/i)?.[0] || '').trim();
      const place = (String(txt).match(/\b(przy|w|na|pod|obok|do)\s+[^,.!?]+/i)?.[0] || '').trim();

      if (purpose) return { question: 'Po co to robię?', answer: purpose };
      if (when)    return { question: 'Kiedy to robię?', answer: when };
    ...
    // === 3. OSOBA — spróbuj heurystyki przed LLM
