...
// === Helper do ładowania verbs.js (obsługuje named i default) ===
async function loadVerbsModule(cacheBust = false) {
  const url = new URL('./verbs.js', import.meta.url).href + (cacheBust ? `?v=${Date.now()}` : '');
  const mod = await import(url);
  return {
    motion:     mod.VERBS_MOTION     || mod.default?.VERBS_MOTION     || [],
    placebound: mod.VERBS_PLACEBOUND || mod.default?.VERBS_PLACEBOUND || [],
    perception: mod.VERBS_PERCEPTION || mod.default?.VERBS_PERCEPTION || [],
    common:     mod.COMMON_VERBS     || mod.default?.COMMON_VERBS     || []
  };
}

// === Startowe ładowanie verbs.js ===
(async () => {
  try {
    const v = await loadVerbsModule(false);
    VERBS_MOTION     = v.motion;
    VERBS_PLACEBOUND = v.placebound;
    VERBS_PERCEPTION = v.perception;
    console.log(`✅ verbs.js: motion=${VERBS_MOTION.length}, place=${VERBS_PLACEBOUND.length}, perc=${VERBS_PERCEPTION.length}, common=${v.common.length}`);
  } catch (e) {
    console.warn("⚠️ Nie udało się załadować ./verbs.js, używam fallbacku:", e.message);
    VERBS_MOTION = ["iść","pójść","chodzić","jechać","pojechać","wracać","wrócić","wejść","wyjść","wsiąść","wysiąść","dojechać","zajechać","dotrzeć","podjechać","odjechać","przyjść","podejść","przejść","lecieć","płynąć","przepłynąć","ruszyć","wędrować","podróżować"];
    VERBS_PLACEBOUND = ["usiąść","siąść","siedzieć","stać","stanąć","leżeć","położyć się","czekać","czytać","pisać","bawić się","grać","jeść","pić","oglądać","rozmawiać","uczyć się","pracować","odpoczywać","spać","rysować"];
    VERBS_PERCEPTION = ["patrzeć","popatrzeć","spoglądać","spojrzeć","przyglądać się","oglądać","zaglądać","zerkać","widzieć","zobaczyć"];
  }
})();
...
(function attachQuizRoutes(app) {
  ...
  const rehydrateFromOriginal = (orig="",frag="") => {
    if(!orig||!frag)return frag||"";
    const O=String(orig),F=normalizeSpaces(frag),Fda=deaccent(F).toLowerCase();
  ...
  const rehydrateAnswerFromOriginal=(orig,a)=>a?stripPunct(rehydrateFromOriginal(orig,a).replace(/\s+/g," ").trim()):a;

  // ————— FLEX & REGEX —————
  function flex(lemma){
    const L=String(lemma||"").toLowerCase();
    const stem=L.replace(/(ć|ść|źć)$/u,"");
    const tails=["","ł","ła","li","ły","łem","łam","łeś","łaś","ył","yła","yli","yły","yłem","yłam","yłeś","yłaś","ę","esz","e","emy","ecie","ą","ąc","ący","ana","any","ane"];
    const altStems={"usiąś":["usiad","usied"],"siąś":["siad","sied"],"wsiąś":["wsiad"],"wysiąś":["wysiad"],"wejś":[" wszed","wesz"],"wyjś":[" wyszed","wysz"],"leż":["leża","leżał","leż"],"jedz":["jad","jedz"]};
    const variants=new Set([stem]); if(altStems[stem]) altStems[stem].forEach(v=>variants.add(v));
    const bodies=[]; for(const v of variants)bodies.push(`${v}(?:${tails.join("|")})?`);
    return `(?:${bodies.join("|")})`;
  }
  ...
  const PURPOSE_TAIL=new RegExp(String.raw`(?:\s+(?:po\s+[^\s,.;!?]+(?:\s+[^\s,.;!?]+){0,3}))|(?:\s+(?:na\s+(?:spacer|spacerze|lekcj[ęeai]|obiad|kolacj[ęe]|lody|zakupy|posiłek|posilek|przerw[ęe])))$`,"iu");
  ...
  const STRIP_ANY_PURPOSE=s=>normalizeSpaces(String(s)
  ...
  const NA_LOCATIVE_BLACKLIST=["na spacerze","na spacer","na dywanie","na trawie","na ławce","na lawce","na przystanku","na basenie","na stadionie","na boisku","na rynku"].map(x=>deaccent(x));

  function extractTime(s){
    const m=String(s).match(/\b(o\s+\d{1,2}[:.]\d{2}|wczoraj|dzisiaj|dziś|jutro|rano|wieczorem|po\s+południu|po\s+poludniu|w\s+(poniedziałek|wtorek|środ[ęe]|czwartek|piątek|sobot[ęe]|niedziel[ęe]))\b/iu);
  ...
  function extractDestination(s){
    ...
    const matches=[];let m;while((m=re.exec(cleaned))!==null)matches.push({prep:m[1].toLowerCase(),body:m[2]});
    ...
    dest=normalizeSpaces(stripPunct(dest));
    const low=deaccent(dest.toLowerCase());
    const head2=deaccent(dest.toLowerCase().split(/\s+/).slice(0,2).join(" "));
    if(dest.toLowerCase().startsWith("na ")&&(NA_LOCATIVE_BLACKLIST.includes(head2)||NA_LOCATIVE_BLACKLIST.includes(low)))return null;
    return dest||null;
  }

  // ===== generator pytania i odpowiedzi =====
  function generateQuestionAndAnswerTeacher(textRaw){
    if(!textRaw||typeof textRaw!=="string")return{ok:false,qtype:"Co?",question:"Co się dzieje?",answer:"",source_path:"error-empty"};
    ...
    function qKiedy(){
      if(time&&/^o\s+\d{1,2}[:.]\d{2}/i.test(time))return"O której godzinie to się wydarzyło?";
      return"Kiedy to się działo?";
    }

    if(dest)return{ok:true,qtype:"Dokąd?",question:qDokad(),answer:rehydrateAnswerFromOriginal(text,dest),source_path:"rule-teacher-strict-v7.6f-fullQ"};
    if(time)return{ok:true,qtype:"Kiedy?",question:qKiedy(),answer:rehydrateAnswerFromOriginal(text,time),source_path:"rule-teacher-strict-v7.6f-fullQ"};
    return{ok:true,qtype:"Co?",question:"Co się dzieje?",answer:"",source_path:"fallback"};
  }
  ...
  app.post("/agent/comprehend-aggregate",(req,res)=>{
    try{
      ...
      const sentences=cleaned.split(/(?<=[.!?])\s+/).slice(0,40);
      const scored=sentences.map(s=>{
        const r=generateQuestionAndAnswerTeacher(s);
        const sc=r.qtype==="Dokąd?"?100:r.qtype==="Kiedy?"?90:50;
        return{...r,sentence:s,score:sc};
      }).sort((a,b)=>b.score-a.score);
      const top=scored.slice(0,max_questions);
  ...
  app.post("/verbs/reload",async(_req,res)=>{
    try{
      const v=await loadVerbsModule(true);
      VERBS_MOTION=v.motion;VERBS_PLACEBOUND=v.placebound;VERBS_PERCEPTION=v.perception;
      console.log(`♻️ verbs.js reloaded (motion=${VERBS_MOTION.length}, place=${VERBS_PLACEBOUND.length}, perc=${VERBS_PERCEPTION.length})`);
      res.json({ok:true,message:"verbs.js reloaded",counts:{motion:VERBS_MOTION.length,placebound:VERBS_PLACEBOUND.length,perception:VERBS_PERCEPTION.length}});
    }catch(err){
  ...
  app.get("/verbs/common",async(_req,res)=>{
    try{
      const v=await loadVerbsModule(false);
      res.json({ok:true,count:v.common.length,verbs:v.common});
    }catch(err){
      res.status(500).json({ok:false,error:String(err.message||err)});
    }
  });

  app.get("/version",(_req,res)=>res.json({build:"2025-10-27 rule-teacher-strict-v7.6f-fullQ-verbsAPI"}));
})(app);
