...
/* ===================== QUIZ: rule-teacher-strict-v7.6f-fullQ — INLINE (safe regex) ===================== */
...
(function attachQuizAndVerbsRoutes(app) {
  ...
  (async () => {
    try {
      ...
      const commonCnt  = (verbsModule.COMMON_VERBS || verbsModule.default || []).length || 0;
      console.log(`✅ verbs.js: motion=${VERBS_MOTION.length}, place=${VERBS_PLACEBOUND.length}, perc=${VERBS_PERCEPTION.length}, common=${commonCnt}`);
    } catch (e) {
  ...
  // ===== Bezpieczne budowanie regexów =====
  function unionFlexRe(list = [], flags = "i") {
    const parts = (list || []).map(flex).filter(Boolean);
    if (!parts.length) return null;                 // <<< KLUCZOWE
    return new RegExp(`\\b(?:${parts.join("|")})\\b`, flags);
  }
  function VERB_CUT_RE() {
    const all = [...VERBS_MOTION, ...VERBS_PLACEBOUND];
    const parts = all.map(flex).filter(Boolean);
    if (!parts.length) return null;                 // <<< KLUCZOWE
    return new RegExp(`\\s+(?:${parts.join("|")})\\b.*$`, "i");
  }
  function SEATLIKE_RE()   { return unionFlexRe(VERBS_PLACEBOUND, "i"); }
  function PERCEPTION_RE() { return unionFlexRe(VERBS_PERCEPTION, "i"); }

  const trimAtVerb = (s = "") => {
    const re = VERB_CUT_RE();
    return re ? normalizeSpaces(String(s).replace(re, "")) : normalizeSpaces(String(s)); // safe no-op
  };
  ...
  function extractDestination(s) {
    ...
    const percRe = PERCEPTION_RE();
    if (pick.prep === "na" && percRe && percRe.test(s)) {
      return null;
    }
    ...
    // jeżeli w dest pojawił się czasownik aktywności, utnij go
  ...
  function pickPlaceFromSentence(s) {
    ...
    const seatRe = SEATLIKE_RE();
    const seatLike = seatRe ? seatRe.test(s) : false;
  ...
  function generateQuestionAndAnswerTeacher(textRaw) {
    ...
    // cue do odmiany („wsiedli”, „wrócili”, „idziemy”)
    ...
    function qKiedy() {
      if (time && /^o\s+\d{1,2}[:.]\d{2}/i.test(time)) {
        if (/\b(wsiad\w*|wsied\w*)\b/i.test(tDA) && /\b(do\s+(autobusu|tramwaju|pociągu|pociagu))\b/i.test(tDA))
    ...
    const seatRe = SEATLIKE_RE();
    const preferPlaceFirst = seatRe ? seatRe.test(t) : false;
  ...
  app.get("/version", (_req, res) =>
    res.json({ build: "2025-10-27 rule-teacher-strict-v7.6f-fullQ-safe" })
  );
