...
const GREETING_TIMEOUT_MS = Number(process.env.GREETING_TIMEOUT_MS || 4500);
const MOTIVATE_TIMEOUT_MS = Number(process.env.MOTIVATE_TIMEOUT_MS || 4500);
const GENERATE_TEXT_TIMEOUT_MS = Number(process.env.GENERATE_TEXT_TIMEOUT_MS || 3500);
...
async function withDeadlineRetry(makePromiseFn, { deadlineMs, retries = 1, backoffMs = 250 }) {
  let ms = deadlineMs; let lastErr;
  for (let i = 0; i <= retries; i++) {
    try { return await withDeadline(makePromiseFn(), ms); }
    catch (e) {
      lastErr = e;
...
function canUseOpenAI() { _pruneOai(); return oaiCallsTimestamps.length < OAI_RPM_LIMIT; }
function markOpenAICall() { _pruneOai(); oaiCallsTimestamps.push(Date.now()); }
...
const MIN_CACHE_GREETINGS = 5;
const MIN_CACHE_TEXTS = 5;
const CACHE_GREETINGS = new Map();
const CACHE_TEXTS = { A1: [], A2: [], B1: [] };
const INFLIGHT_G = new Set();
const INFLIGHT_T = new Set();

function _keyG(age, character) { return `${Number(age) || 'X'}|${character || 'Bohater'}`; }
function cachePushGreeting(age, character, text) {
  const k = _keyG(age, character);
  const arr = CACHE_GREETINGS.get(k) || [];
  if (text && !arr.includes(text)) { arr.push(text); CACHE_GREETINGS.set(k, arr); }
}
function cacheShiftGreeting(age, character) {
  const k = _keyG(age, character);
  const arr = CACHE_GREETINGS.get(k) || [];
  const out = arr.shift();
  CACHE_GREETINGS.set(k, arr);
  ensureGreetingStock(age, character).catch(() => {});
  return out || null;
}
function cachePushText(level, text) {
  const L = String(level || 'A1').toUpperCase();
  if (!CACHE_TEXTS[L]) CACHE_TEXTS[L] = [];
  if (text && !CACHE_TEXTS[L].includes(text)) CACHE_TEXTS[L].push(text);
}
function cacheShiftText(level) {
  const L = String(level || 'A1').toUpperCase();
  const arr = CACHE_TEXTS[L] || [];
  const out = arr.shift() || null;
  ensureTextStock(L).catch(() => {});
  return out;
}
...
const WHITELIST =
  ...
  'abcdefghijklmnopqrstuvwxyząćęłńóśźż' + '0123456789' +
  ' .,:;!?„”"\'()-–—/\\[]{}…';
...
/* ===== AUDIO helpers ===== */
const EXT_BY_MIME = {
  'audio/webm': 'webm', 'audio/m4a': 'm4a', 'audio/mp4': 'mp4',
...
/* ===== Root & health ===== */
app.get('/health', (_req, res) => res.json({ ok: true, version: '1.20-cache5' }));
app.get('/', (_req, res) => {
  res.type('html').send(`<html><body><h3>poczytajmy-backend</h3>
    <ul><li>/agent/generate-greeting</li><li>/agent/generate-text</li></ul></body></html>`);
});
...
function looksTemplatey(s = '') {
  ...
  if (/^Dziś|^Dzisiaj|^Po południu|^W ogrodzie/.test(t)) return true;
  const conj = (t.match(/\b(a|oraz)\b/gi) || []).length;
  if (conj >= 2) return true;
  return false;
}
/* ===== Polish helpers (sanitize/soften/novelty) ===== */
...
function rememberText(t) {
  recentTexts.unshift(String(t));
  if (recentTexts.length > 20) recentTexts.pop();
}
...
async function groqChat({ messages, max_tokens = MAX_TOKENS_FAST, temperature = 0.3, top_p = 0.95 }) {
  ...
  const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
    ...
    body: JSON.stringify({ model: GROQ_CHAT_MODEL, temperature, top_p, max_tokens, messages }),
...
async function openaiChat({ messages, max_tokens = MAX_TOKENS_FAST, temperature = 0.3, top_p = 0.95 }) {
  ...
  if (!canUseOpenAI()) {
    const e = new Error('OPENAI_THROTTLED'); e.code = 'OPENAI_THROTTLED'; throw e;
  }
  const t0 = now();
  try {
    const r = await openai.chat.completions.create({
      model: 'gpt-4o-mini', messages, temperature, top_p, max_tokens,
      frequency_penalty: NAT_FREQ_PENALTY, presence_penalty: NAT_PRES_PENALTY,
    });
    markOpenAICall();
...
async function chatPref({ prompt, max_tokens = 150, temperature = 0.3, top_p = 0.95, deadlineMs = DEADLINE_MS }) {
  ...
  try {
    if (openai) return await withDeadlineRetry(makeOai, { deadlineMs, retries: 1, backoffMs: 250 });
  } catch (err) { if (err?.code !== 'OPENAI_THROTTLED') throw err; }
  if (groq) {
    try {
      return await withDeadline(
        groqChat({ messages, max_tokens, temperature: Math.min(0.7, Math.max(0.2, temperature)), top_p }),
...
function buildGreetingPrompt({ age, character = 'Twój przyjaciel', theme = '', n = 12 }) {
  ...
  const tone =
    Number.isFinite(age) && age <= 5 ? 'proste, ciepłe, zabawowe; rytm mowy dziecka; onomatopeje OK'
    : Number.isFinite(age) && age <= 8 ? 'żywe, motywujące; mini-misja; 1–2 emoji'
  ...
  return `Wymyśl ${n} ZUPEŁNIE różnych, krótkich powitań po polsku dla dziecka (wiek: ${wiek}). Mówi ${character}. Styl: ${tone}. ${heroHint}
Temat przewodni: ${chosenTopic}.
⚡ Każde powitanie MUSI odnosić się do czytania i książek (np. książka, rozdział, bajka, narrator, zakładka, biblioteka, opowieść, litery).
⚡ NIE używaj motywów typu: las, bieganie, sport, piknik, podróże — tylko świat książek.
⚡ Zakaz: brak słów powitalnych (cześć/hej/witaj/siema/halo) i brak imienia dziecka.
Zasady: jedno zdanie, 6–14 wyrazów, bez cudzysłowów i bez wstępów. Każde powitanie w osobnej linii poprzedzone myślnikiem "- ".`;
}

/* ===== Cache loaders & keep-5 ===== */
async function fillGreetingBucket({ age, character, theme, n = 8 }) {
  try {
    const prompt = buildGreetingPrompt({ age: Number(age), character, theme, n });
    const r = await chatPref({
      prompt, temperature: NAT_TEMPERATURE, max_tokens: 220, top_p: NAT_TOP_P,
      deadlineMs: Math.min(GREETING_TIMEOUT_MS, 3500),
    });
    const raw = r?.text || '';
    let cands = parseList(raw);
    if (!cands.length && raw) cands = raw.split(/[.\n]/).map(s => s.trim()).filter(Boolean);
    for (const cand of cands) {
      const cleaned = softenPolish(sanitizeNoName('', cand));
      cachePushGreeting(age, character, cleaned);
    }
  } catch {}
}
async function fillTextBucket(level = 'A1', n = 12) {
  try {
    const prompt = `Napisz ${n} KRÓTKICH zdań (po polsku) do głośnego czytania (poziom ${String(level).toUpperCase()}).
Zasady: 8–16 słów, codzienne słownictwo, ZERO cudzysłowów i nawiasów, bez „a potem”. Każde zdanie w nowej linii.`;
    const r = await chatPref({
      prompt, temperature: NAT_TEMPERATURE, max_tokens: 600, top_p: NAT_TOP_P,
      deadlineMs: Math.min(GENERATE_TEXT_TIMEOUT_MS, 3200),
    });
    const lines = (r?.text || '')
      .split(/\r?\n/)
      .map(s => softenPolish(cleanSentence(s)))
      .filter(Boolean);
    for (const s of lines) {
      const v = validateKidsSentencePL(s);
      if (v.ok && !looksTemplatey(v.text) && !tooSimilarToRecent(v.text)) cachePushText(level, v.text);
    }
    while ((CACHE_TEXTS[String(level).toUpperCase()] || []).length < MIN_CACHE_TEXTS) {
      cachePushText(level, pick(bankByLevel(level)));
    }
  } catch {
    while ((CACHE_TEXTS[String(level).toUpperCase()] || []).length < MIN_CACHE_TEXTS) {
      cachePushText(level, pick(bankByLevel(level)));
...
async function ensureGreetingStock(age, character) {
  const key = _keyG(age, character);
  const arr = CACHE_GREETINGS.get(key) || [];
  if (arr.length >= MIN_CACHE_GREETINGS) return;
  if (INFLIGHT_G.has(key)) return;
  INFLIGHT_G.add(key);
  try {
    const need = MIN_CACHE_GREETINGS - arr.length;
    const theme = HERO_THEMES[character] || '';
    await fillGreetingBucket({ age, character, theme, n: Math.max(6, need * 2) });
  } finally { INFLIGHT_G.delete(key); }
}
async function ensureTextStock(level = 'A1') {
  const L = String(level).toUpperCase();
  const arr = CACHE_TEXTS[L] || [];
  if (arr.length >= MIN_CACHE_TEXTS) return;
  if (INFLIGHT_T.has(L)) return;
  INFLIGHT_T.add(L);
  try {
    const need = MIN_CACHE_TEXTS - arr.length;
    await fillTextBucket(L, Math.max(8, need * 2));
  } finally { INFLIGHT_T.delete(L); }
}

/* ===== Greeting generation (live, used if cache empty) ===== */
const recentGreetings = new Map();
async function generateGreetingV2({ name, age, character, theme }) {
  // Jeden szybki strzał; w razie porażki – krótki fallback.
  let raw = '';
  try {
    const r = await chatPref({
      prompt: buildGreetingPrompt({ age: Number(age), character, theme, n: 10 }),
      temperature: NAT_TEMPERATURE, max_tokens: 160, top_p: NAT_TOP_P,
  ...
  if (!cands.length && raw) cands = raw.split(/[.\n]/).map(s => s.trim()).filter(Boolean);
  if (!cands.length) {
    return { text: 'Zajrzymy dziś do książki i wyszukamy nowe słowa.', source: 'fallback-bank' };
  }
...
const BANK_A2 = [
  ...
  'W bibliotece cicho przewracamy strony nowej książki.',
];
const BANK_B1 = [
  ...
  'Cierpliwie wyszukuję trudniejsze słowa i czytam je powoli.',
];
...
function countWords(s) { return (String(s).trim().match(/\b[\p{L}\p{M}0-9'-]+\b/gu) || []).length; }
const PROFANITY = ['kurwa','cholera','debil','idiota','głupi','szmata','pedał','lesba','spier','nienawidzę','zabij','śmierć'];
function hasForbidden(s) { const low = String(s).toLowerCase(); return PROFANITY.some(p => low.includes(p)); }
function hasPolishDiacritics(s) { return /[ąćęłńóśźż]/i.test(String(s)); }
function validateKidsSentencePL(s, { minWords = 8, maxWords = 16 } = {}) {
  const issues = []; const txt = cleanSentence(onlyOneSentence(s)); const words = countWords(txt);
  if (words < minWords || words > maxWords) issues.push(`Liczba słów ${words} poza zakresem ${minWords}–${maxWords}.`);
...
app.post('/agent/generate-greeting', async (req, res) => {
  try {
    ...
    // cache-first
    const cached = cacheShiftGreeting(age, character);
    if (cached) return res.json({ ok: true, text: cached, source: 'cache' });
    ...
    return res.status(200).json({ ok: true, text: 'Zajrzymy dziś do książki i wyszukamy nowe słowa.', source: 'fallback' });
  }
});
...
/* ===== Endpoint: generate-text (cache-first) ===== */
async function handleGenerateText(req, res) {
  try {
    const { language = 'pl', level = 'A1' } = req.body || {};
    // cache-first
    const hit = cacheShiftText(level);
    if (hit) {
      rememberText(hit);
      ensureTextStock(level).catch(()=>{});
      return res.json({ ok: true, text: hit, level, language, source: 'cache' });
    }

    const prompt = `Napisz jedno naturalne i lekkie zdanie po polsku do głośnego czytania przez dziecko (poziom ${String(level).toUpperCase()}).
Wymagania:
...
Podaj tylko gotowe zdanie.`;
    ...
    try {
      const r = await chatPref({
        prompt, temperature: NAT_TEMPERATURE, max_tokens: 70, top_p: NAT_TOP_P,
        deadlineMs: GENERATE_TEXT_TIMEOUT_MS,
      });
      sentence = softenPolish(cleanSentence(r.text || ''));
    } catch { sentence = ''; }
    ...
    if (!sentence) {
      ...
      rememberText(backup);
      ensureTextStock(level).catch(()=>{});
      return res.json({ ok: true, text: backup, level, language, source: 'timeout-fallback' });
    }

    const check = validateKidsSentencePL(sentence);
    ...
    ensureTextStock(level).catch(()=>{});
    return res.json({ ok: true, text: check.text, level, language, source: 'llm' });
  } catch (err) {
    console.error('agent/generate-text error:', err);
    const { level = 'A1', language = 'pl' } = req.body || {};
    const backup = pick(bankByLevel(level));
    ensureTextStock(level).catch(()=>{});
    return res.status(200).json({ ok: true, text: backup, level, language, source: 'error-fallback' });
  }
}
...
/* ===== Manual preload ===== */
app.post('/agent/preload', async (req, res) => {
  try {
    const { greetings = [], texts = [] } = req.body || {};
    for (const g of greetings) ensureGreetingStock(g.age, g.character).catch(()=>{});
    for (const t of texts) ensureTextStock(t.level || 'A1').catch(()=>{});
    res.json({ ok: true });
  } catch { res.json({ ok: true }); }
});
/* ===================== ASR (transkrypcja z timestampami) ===================== */
  ...
  try {
    ...
    function norm(s = '') {
      return String(s).toLowerCase()
        .replace(/[^\p{L}\p{M}0-9\s]+/gu, ' ').replace(/\s+/g, ' ').trim();
    }
    function jacc(a, b) {
    ...
    return res.json({ ok: true, recognizedText, wordsRead, accuracy, wordTimestamps, source: provider });
  } catch (err) {
...
app.post('/tts', async (req, res) => {
  try {
    ...
    const { text = '', voiceId = DEFAULT_ELEVEN_VOICE_ID, output_format = 'mp3_44100_128',
      stability = 0.5, similarity_boost = 0.75 } = req.body || {};
    ...
    const r = await fetch(`https://api.elevenlabs.io/v1/text-to-speech/${voiceId}`, {
      ...
      body: JSON.stringify({
        text: clean, model_id: 'eleven_multilingual_v2', output_format,
        voice_settings: { stability, similarity_boost },
      }),
      agent: keepAliveAgent,
    });
    if (!r.ok) {
      let details = ''; try { details = await r.text(); } catch {}
...
// uproszczona, stabilna detekcja miejsca/czasu/celu
const RE_PLACE = /\b(?:w|we|na|do|przy|pod|u|obok)\s+(?!celu\b)(?!\s)(?!i\b|oraz\b|ale\b|a\b)([^\s0-9.,;!?]+(?:\s+(?!i\b|oraz\b|ale\b|a\b)[^\s0-9.,;!?]+){0,2})/iu;
...
function trimPlace(p='') {
  let toks = String(p).trim().split(/\s+/).filter(w=>!/^(i|oraz|ale|a|potem)$/i.test(w)).slice(0,3);
...
function buildTeacherPrompt(text, maxQ=3){
  ...
  return `Jesteś nauczycielem języka polskiego (klasy 1–3). Na podstawie TEKSTU przygotuj od 1 do ${clamped} PROSTYCH pytań i krótkich poprawnych odpowiedzi (max 6 słów) — WYŁĄCZNIE z treści.
Zasady:
...
- „Po co…?”/„Dlaczego…?” tylko jeśli w TEKŚCIE występują markery celu (żeby, aby, bo, ponieważ, dlatego że, w celu, po to).
- Dozwolone formy: „Kto…?”, „Co robi…?”, „Gdzie…?”, „Kiedy…?”, „Po co…?” (warunkowo).
- Pytania krótkie (max 8 słów), odpowiedzi 1–6 słów, polskie znaki.
Zwróć dokładnie JSON: {"questions":[{"question":"…?","answer":"…"}]}
TEKST: """${qz_trim(text, 900)}"""`;
}
...
function postProcessLLMItems(items, text, want = 3) {
  const out = []; const placeMatch = (text.match(RE_PLACE)||[])[0];
  for (const it of items || []) {
    let q = cleanQuestion(it.question || ''); let a = (it.answer || '').trim();
    ...
    if (RE_PURPOSE.test(a) && !/^\s*(Po co|Dlaczego)\b/i.test(q)) q = 'Po co?';
...
async function llmQuestionsRace(text, countHint){
  ...
  const oaiPromise = (async () => {
    ...
    const r = await withDeadlineRetry(
      () => openai.chat.completions.create({
        model: 'gpt-4o-mini', temperature: 0.2, max_tokens: 180, top_p: 0.95,
  ...
  const groqPromise = (async () => {
    ...
    const r = await hardTimeout(
      groqChat({ messages: [{ role: 'user', content: prompt }], max_tokens: 180, temperature: 0.2, top_p: 0.95 }),
  ...
  try {
    const res = await hardTimeout(Promise.any([oaiPromise, groqPromise]), COMPREHEND_TIMEOUT_MS);
...
app.post('/agent/comprehend-multi', async (req, res) => {
  ...
  try {
    ...
    let out = []; let provider = null;

    const race = await llmQuestionsRace(src, want);
    provider = race.provider;
    out = postProcessLLMItems(race.items, src, want);

    if (out.length < want) out = out.concat(ruleFallback(src, want - out.length));
    if (out.length < want) out = out.concat(heuristicMulti(src, want - out.length));
  ...
  try {
    ...
    if (pp.length) {
      qa = {
        ...
        fallback: false, sentence: bestSent, source_path: 'llm+post', llm_provider: race.provider
      };
    } else {
...
async function prewarmOnce() {
  try {
    ...
    // Opcjonalnie: rozruch cache domyślnych profili/poziomów
    ensureGreetingStock(7, 'Twój przyjaciel').catch(() => {});
    ensureTextStock('A1').catch(() => {});
    ensureTextStock('A2').catch(() => {});
    ensureTextStock('B1').catch(() => {});
  } catch { /* noop */ }
}
...
app.listen(PORT, () => {
  ...
  if (PREWARM_EVERY_MIN > 0) {
    ...
    console.log(
      `🛌 Anti-sleep: ping co ${PREWARM_EVERY_MIN} min${BASE_URL ? ` → ${BASE_URL}/health` : ''}`
    );
  }
});
