...
// ===================== QUIZ: rule-teacher-strict-v7.6f-fullQ (ESM) =====================

/**
 * Ten moduł zakłada, że pracujesz w ESM ("type": "module" w package.json).
 * Eksportuje funkcję registerQuizRoutes(app), która rejestruje endpointy:
 *   POST /agent/comprehend
 *   POST /agent/comprehend-aggregate
 *   GET  /version
 *
 * Użycie (w Twoim index.js):
 *   import express from "express";
 *   import { registerQuizRoutes } from "./quiz-routes.js"; // <- jeśli wydzielisz do osobnego pliku
 *   const app = express();
...
// ----- (opcjonalnie) standalone bootstrap — odkomentuj tylko jeśli nie masz jeszcze app -----
// import express from "express";
// if (!globalThis.__QUIZ_STANDALONE__) {
//   const app = express();
//   app.use(express.json({ limit: "1mb" }));
//   registerQuizRoutes(app);
//   const PORT = process.env.PORT || 10000;
//   app.listen(PORT, () => console.log("Quiz API listening on " + PORT));
//   globalThis.__QUIZ_STANDALONE__ = true;
// }
// ===================== /QUIZ / COMPREHEND (ESM) =====================
