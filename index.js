...
async function acquire() {
  while (inflight >= MAX_CONCURRENCY) await sleep(40);
  inflight++;
}
function release() {
...
app.get('/', (_req, res) => {
  res.type('html').send(`
    <html>
      <head><meta charset="utf-8"><title>poczytajmy-backend</title></head>
...
function buildGreetingPrompt({ age, character = 'Twój przyjaciel', theme = '', n = 12 }) {
  ...
  const tone = Number.isFinite(age) && age <= 5
...
function rubricByAccuracy(acc) {
  ...
  if (s >= 95)
    return 'wynik świetny; podkreśl perfekcję i zaproponuj trudniejsze słowo przy następnej stronie';
  if (s >= 80)
    return 'wynik bardzo dobry; pochwal płynność i zaproponuj jedną mikro-radę (np. dokładniej końcówki)';
  if (s >= 60)
    return 'wynik dobry; pochwal staranie i podaj jedną prostą wskazówkę (np. wolniej, sylabizuj trudniejsze słowa)';
...
function buildMotivationPrompt({ age, accuracy, text, characterName = 'Bohater', lang = 'pl' }) {
  ...
  return `Jesteś ${characterName} w aplikacji do czytania. Napisz 1 naturalny, krótki komentarz po polsku.
Styl:
- ${tone}
- ${rubric}
...
Kontekst (nie cytuj literalnie, możesz nawiązać ogólnie): "${excerpt}"
...
async function generateMotivation({ age, accuracy, text, characterName, lang = 'pl' }) {
  ...
  try {
    const r = await withDeadlineRetry(makeCall, {
      deadlineMs: Math.min(MOTIVATE_TIMEOUT_MS || 9000, 9000),
      retries: 1,
      backoffMs: 350,
    });
  ...
  if (looksTemplatey(out) || tooSimilarToRecent(out)) {
    try {
      const r2 = await withDeadlineRetry(makeCall, {
        deadlineMs: Math.min(MOTIVATE_TIMEOUT_MS || 9000, 9000),
        retries: 0,
      });
...
// handler generatora tekstu
async function handleGenerateText(req, res) {
  try {
    const { language = 'pl', level = 'A1' } = req.body || {};

    const prompt = `Napisz jedno naturalne i lekkie zdanie po polsku do głośnego czytania przez dziecko (poziom ${String(
      level
    ).toUpperCase()}). Wymagania:
...
Podaj tylko gotowe zdanie.`;
    ...
    if (!check.ok || looksTemplatey(check.text) || tooSimilarToRecent(check.text)) {
      const fixed = cleanSentence(
        softenPolish(
          await chatPref({
            prompt: `Uprość i „rozluźnij” zdanie dla dziecka (naturalny język, 8–16 słów, bez cudzysłowów i „a potem”): ${sentence}`.trim(),
            temperature: 0.6,
            max_tokens: 60,
            top_p: NAT_TOP_P,
            deadlineMs: GENERATE_TEXT_TIMEOUT_MS,
      ...
      if (check2.ok && !looksTemplatey(check2.text) && !tooSimilarToRecent(check2.text)) {
        ...
        return res.json({
          ...
          level,
          language,
          source: `${first.provider}+soft-corrector`,
        });
      }
...
app.post('/tts', async (req, res) => {
  try {
    ...
    if (!r.ok) {
      ...
      return res
        .status(r.status)
        .json({ ok: false, error: `ELEVEN_HTTP_${r.status}`, details: details?.slice(0, 800) });
    }
    ...
    res
      .status(500)
      .json({ ok: false, error: 'TTS_PROXY_FAILED', details: String(err?.message || err) });
  }
});
...
app.get('/tts-voices', async (_req, res) => {
  try {
    ...
    const voices = Array.isArray(data?.voices)
      ? data.voices.map((v) => ({ id: v.voice_id, name: v.name }))
      : [];
    ...
    return res
      .status(500)
      .json({ ok: false, error: 'VOICES_FAILED', details: String(err?.message || err) });
  }
});
...
// uproszczona, stabilna detekcja miejsca: przyimek + do 3 słów (z wykluczeniami spójników)
const RE_PLACE = /\b(?:w|we|na|do|przy|pod|u|obok)\s+(?!celu\b)(?!\s)(?!i\b|oraz\b|ale\b|a\b)([^\s0-9.,;!?]+(?:\s+(?!i\b|oraz\b|ale\b|a\b)[^\s0-9.,;!?]+){0,2})/iu;
const RE_TIME =
  /\b(rano|wieczorem|w południe|po południu|wczoraj|dzisiaj|dziś|jutro|po obiedzie|w niedzielę|później)\b/iu;
...
const PLACE_VERBS =
  /(jest|stoi|leży|lezy|idzie|biegnie|wieje|patrzy|pisze|czyta|maluje|rysuje|słucha|slucha|gra|je|pije)$/i;
...
function heuristicQA(sentence) {
  ...
  const subj = (text.match(/^([A-ZŁŚŻŹĆŃÓ][\p{L}\p{M}\-']+)/iu) || [])[1] || ''; // pierwsze słowo
  const placeRaw = (text.match(RE_PLACE) || [])[0];
  const place = placeRaw ? trimPlace(placeRaw) : '';
  const time = (text.match(RE_TIME) || [])[0];
  const mVerb =
    (text.match(
      /\b(śpi|spi|czyta|pisze|rysuje|maluje|je|pije|ogląda|oglad|słucha|slucha|idzie|biegnie|gra|stoi|siedzi|leży|lezy|patrzy)\b/iu
    ) || [])[0];
...
/* porządkowanie bzdurek */
function dedupeVerbInQuestion(q) {
  if (!q) return q;
  let s = String(q).replace(/\s+/g, ' ').trim();
  s = s.replace(
    /\b(czyta|pije|je|gra|idzie|biegnie|stoi|siedzi|leży|lezy|ogląda|oglad|pisze|rysuje|maluje)\s+\1\b/gi,
    '$1'
  );
  s = s.replace(
    /\b(ksiazke|książkę|kakao|zup[eęa]|komiks|piłk[ae]|muzyke|muzykę)\s+(czyta|pije|je|gra)\?/gi,
    '$2?'
  );
  return s;
}
...
function postProcessLLMItems(items, text, want = 3) {
  ...
  for (const it of items || []) {
    ...
    if (/^\s*Gdzie\b/i.test(q)) {
      if (RE_PURPOSE.test(a)) {
        if (placeMatch) a = trimPlace(placeMatch);
        else continue;
      }
      if (!RE_PLACE_ANS_START.test(a)) {
        if (placeMatch) a = trimPlace(placeMatch);
        else continue;
      }
    }
...
/* równoległy wyścig: OpenAI i Groq */
...
app.post('/agent/comprehend-multi', async (req, res) => {
  const dbg =
    flag(req.query?.debug) || flag(req.body?.debug) || COMPREHEND_DEBUG;
  try {
    ...
    // 4) Uzupełnienia i cięcie do want
