...
// [UWAGA] Używa stałych z góry pliku: POOL_LEVELS, POOL_TARGET_SIZE, POOL_REFILL_BATCH, POOL_REFILL_INTERVAL_MS

/* Prefetch provider do zadań w tle (opcjonalnie) */
...
/* ——— generator jednego zdania do poola ——— */
...
Podaj tylko gotowe zdanie.`;
  ...
  if (!v.ok || _tp_looksTemplatey(v.text) || _tp_tooSimilarToRecent(v.text)) {
    ...
    if (!v.ok || _tp_looksTemplatey(v.text) || _tp_tooSimilarToRecent(v.text)) {
      ...
      s = bank[Math.floor(Math.random()*bank.length)]; // ostateczny fallback
    } else {
      s = v.text;
    }
  } else {
  ...
  try {
    for (const lv of levels) {
      ...
      for (let i = 0; i < want; i++) {
        ...
        await sleep(150); // oddech dla providerów
      }
...
function cleanQuestion(q) {
  ...
  /* >>> DODANE 2 LINIE (normalizacja dziwnych konstrukcji) <<< */
  // "Gdzie jest Na/Po/W/We …" → neutralne
  s = s.replace(/^Gdzie\s+jest\s+(Na|Po|W|We)\b.*$/iu, 'Gdzie to było?');
  // "Gdzie … Potem …" → sygnał czasu, więc "Kiedy to było?"
...
async function prewarmOnce() {
  try {
    ...
    // 🔥 uzupełnij pulę tekstów przy starcie
...
app.listen(PORT, () => {
  ...
  // ⏱ cykliczne uzupełnianie puli tekstów
