...
# 1) Jednorazowe sanity – "Potem"
Invoke-POST-Debug -Uri "$BASE/agent/comprehend-multi?debug=1" -Body @{ text = "Janek biegnie do autobusu. Potem siada przy oknie."; age=8; count=3 }

# 2) "Na spacerze … Wieczorem …"
Invoke-POST-Debug -Uri "$BASE/agent/comprehend-multi?debug=1" -Body @{ text = "Na spacerze Kuba zbiera kasztany. Wieczorem pokazuje je mamie."; age=8; count=3 }

# 3) Krótka pętla po kilku tricky zdaniach
$cases = @(
  "Rano Ala ćwiczy na boisku. Potem biegnie do klasy.",
  "Na spacerze Kuba zbiera kasztany. Wieczorem pokazuje je mamie.",
  "Po kolacji Ola sprząta zabawki. Potem myje zęby."
)
foreach($t in $cases){
  $r = Invoke-ComprehendMulti -Text $t -Age 8 -Count 3
  "$t`n" + ($r.items | ForEach-Object { " - Q: {0} | A: {1}" -f $_.question, $_.answer }) + "`n"
}
